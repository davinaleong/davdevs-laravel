...
**Image asset audit (2026-07-07):** 212 total image files under `public/` — 159 belong to the 8 in-scope content types (articles 62, projects 65, fem 26, knowledge-sharing 4, static 2), 52 belong to the 6 deferred ebooks under `public/books/**`, and 1 (`og.png`) is a site-wide default OG image referenced from the old site's root `layout.tsx`, not tied to any entry. `tools`/`notebooks`/`sermons` have zero images. One orphan file was found and is not imported: `projects/20220811-001-5-html-css-js-projects (2).png` — a duplicate-download artifact not referenced by any frontmatter or body content (the real file, without ` (2)`, is what `20220811-5-coding-projects.md` actually points to).

**Cloudinary naming convention (2026-07-07):** `CloudinaryService::upload()` now accepts an optional `publicId` (a slash-delimited path relative to the existing `davdevs` root folder). `migration:import-images` sets it to `entries/{content_type_slug}/{slugified-old-filename}` — e.g. `davdevs/entries/article/20231229-0001-delving-into-ai`. This makes every migrated asset identifiable by content type + the old site's own date/slug-bearing filename directly in the Cloudinary console, with no need to cross-reference the database. It intentionally does **not** key off the entry's new system-generated slug (§7.2) — that slug doesn't exist until `migration:import-entries` runs afterward, and coupling the two commands' ordering wasn't worth it when the old filename is already a stable, human-readable identifier. The CMS's standalone Image Manager upload (`ImageController`) is unaffected — it has no owning entry at upload time, so it keeps falling back to the flat `davdevs` folder with an auto-generated ID. Ebooks, when unblocked, should follow the same shape: `davdevs/publications/ebook/{book-slug}/...`.
