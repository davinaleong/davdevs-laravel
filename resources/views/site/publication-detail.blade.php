...
<x-site-layout :title="$publication->title" :description="$publication->excerpt" :og-image="$publication->ogImage?->url ?? $publication->coverImage?->url" :json-ld="$jsonLd" :favicon-image-id="$publication->getMeta('favicon_image_id')">
    <article style="max-width:680px;margin:0 auto;padding:48px 36px;">
        ...
        @endif
        <h1
            style="font-family:'Syne',sans-serif;font-size:26px;font-weight:800;color:var(--text-primary);margin:0 0 8px;">
            {{ $publication->title }}</h1>
        @if ($publication->tagline)
            <p style="font-family:'Inter',sans-serif;font-size:15px;color:var(--text-secondary);margin:0 0 24px;">
                {{ $publication->tagline }}</p>
        @endif

        <div class="prose"
            style="font-family:'Lora',serif;font-size:14px;line-height:1.8;color:var(--text-body);margin-bottom:32px;">
            @markdown($publication->body)</div>
        ...
        @if ($publication->bundle && $publication->bundleMembers->isNotEmpty())
            <div style="margin-top:32px;">
                <h2 style="font-family:'Syne',sans-serif;font-size:16px;font-weight:700;color:var(--text-primary);">
                    Included in this bundle</h2>
                <ul>
                    @foreach ($publication->bundleMembers as $member)
                        <li><a href="{{ route('site.ebooks.show', $member->slug) }}"
                                style="color:var(--accent);">{{ $member->title }}</a></li>
                    @endforeach
                </ul>
            </div>
        @endif
