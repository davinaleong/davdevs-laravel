...
<a href="{{ route('site.ebooks.show', $pub->slug) }}" style="background:var(--bg-page);padding:18px;text-decoration:none;display:block;">
    ...
    @if($displayPrice && $pub->store)<div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--accent);margin-top:6px;">{{ $pub->store->price_display }}</div>@endif
</a>
