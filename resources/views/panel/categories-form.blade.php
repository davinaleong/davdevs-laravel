<x-cms-layout title="{{ $category->exists ? 'Edit Category' : 'New Category' }}">
    ...
    <form method="POST" action="{{ $category->exists ? route('panel.categories.update', $category) : route('panel.categories.store') }}" style="max-width:520px;">
        ...
        <div style="display:flex;gap:10px;margin-top:16px;">
            ...
            <a href="{{ route('panel.categories.index') }}" style="background:var(--cms-btn-secondary-bg);border:1px solid var(--cms-btn-secondary-border);color:var(--cms-btn-secondary-text);padding:10px 20px;border-radius:5px;font-size:13px;text-decoration:none;">Cancel</a>
        </div>
    </form>
</x-cms-layout>
