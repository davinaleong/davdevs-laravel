<x-cms-layout title="{{ $contentType->exists ? 'Edit Content Type' : 'New Content Type' }}">
    ...
    <form method="POST" action="{{ $contentType->exists ? route('panel.content-types.update', $contentType) : route('panel.content-types.store') }}" style="max-width:520px;">
        ...
        <div style="display:flex;gap:10px;margin-top:16px;">
            ...
            <a href="{{ route('panel.content-types.index') }}" style="background:var(--cms-btn-secondary-bg);border:1px solid var(--cms-btn-secondary-border);color:var(--cms-btn-secondary-text);padding:8px 16px;border-radius:5px;font-size:13px;text-decoration:none;">Cancel</a>
        </div>
    </form>
</x-cms-layout>
