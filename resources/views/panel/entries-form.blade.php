    ...
    <div x-data="{
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
            <h1
                ...
                {{ $entry->exists ? 'Edit Entry' : 'New Entry' }}</h1>
            @if ($entry->exists)
                <span style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--cms-text-muted);">Read
                    time: {{ $entry->read_time }} min · Slug: {{ $entry->slug }}</span>
            @endif
        </div>

        <form method="POST"
            action="{{ $entry->exists ? route('panel.entries.update', $entry) : route('panel.entries.store') }}">
            @csrf
            @if ($entry->exists)
                @method('PUT')
            @endif
            ...
            <div x-show="tab === 'content'">
                <div
                    ...
                    <div>
                        ...
                        @error('title')
                            <p style="color:var(--cms-error);font-size:12px;margin-top:4px;">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
                    ...
                    <div>
                        ...
                        <div x-show="aiSuggestions && aiSuggestions.length" x-cloak
                            ...
                            <div
                                style="font-size:11px;font-weight:600;color:var(--cms-text-primary);margin-bottom:8px;">
                                AI Audit Suggestions</div>
                            <template x-for="s in aiSuggestions">
                                <div style="display:flex;gap:8px;margin-bottom:6px;font-size:12px;">
                                    <span
                                        style="font-family:'JetBrains Mono',monospace;font-size:9px;padding:2px 6px;border-radius:3px;background:var(--cms-accent-tint);color:var(--cms-accent);white-space:nowrap;height:fit-content;"
                    ...
                    <div
                        ...
                        <div style="max-height:260px;overflow-y:auto;display:flex;flex-direction:column;gap:6px;">
                            @foreach ($images as $img)
                                <label
                                    style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:4px;border-radius:4px;"
                                    :style="selectedImages.includes({{ $img->id }}) ?
                                    ...
                                    <span
                                        style="font-size:12px;color:var(--cms-text-secondary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $img->title ?: $img->alt ?: 'Untitled' }}</span>
                                </label>
                            @endforeach
                        </div>
                        <a href="{{ route('panel.images.create') }}" target="_blank"
                            style="font-size:11px;color:var(--cms-accent);text-decoration:none;display:block;margin-top:8px;">+
                            Upload new image</a>
                    </div>
                    ...
                    <div
                        ...
                        <div style="max-height:260px;overflow-y:auto;display:flex;flex-direction:column;gap:6px;">
                            @foreach ($videoEmbeds as $v)
                                <label
                                    style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:4px;border-radius:4px;">
                                    <input type="checkbox" value="{{ $v->id }}" x-model="selectedVideos"
                                        name="video_embeds[]" style="accent-color:var(--cms-accent);">
                                    <img src="{{ $v->thumbnail_url }}"
                                        style="width:40px;height:24px;object-fit:cover;border-radius:3px;">
                                    <span
                                        style="font-size:12px;color:var(--cms-text-secondary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $v->title }}</span>
                                </label>
                            @endforeach
                        </div>
                        <a href="{{ route('panel.video-embeds.create') }}" target="_blank"
                            style="font-size:11px;color:var(--cms-accent);text-decoration:none;display:block;margin-top:8px;">+
                            Add new video</a>
                    </div>
            ...
            <div x-show="tab === 'meta'" x-cloak>
                <div
                    ...
                    <div style="font-size:13px;font-weight:600;margin-bottom:10px;color:var(--cms-text-primary);">
                        Custom Meta Fields</div>
                    <p style="font-size:11px;color:var(--cms-text-muted);margin-bottom:12px;">e.g. github_repo_url,
                        live_demo_url, fem_difficulty, sermon_speaker, lighthouse_performance</p>
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                        <div>
                            <label
                                ...
                                Type</label>
                            <select name="content_type_id" x-model="contentTypeId" required
                                ...
                                @foreach ($contentTypes as $ct)
                                    <option value="{{ $ct->id }}"
                                        {{ old('content_type_id', $entry->content_type_id) == $ct->id ? 'selected' : '' }}>
                                        {{ $ct->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div x-show="reactIslandByType[contentTypeId]" x-cloak>
                            ...
                            <select name="react_component_id"
                                ...
                                @foreach ($reactComponents as $rc)
                                    <option value="{{ $rc->id }}"
                                        {{ old('react_component_id', $entry->react_component_id) == $rc->id ? 'selected' : '' }}>
                                        {{ $rc->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="category_id"
                                ...
                                @foreach ($categories as $c)
                                    <option value="{{ $c->id }}"
                                        {{ old('category_id', $entry->category_id) == $c->id ? 'selected' : '' }}>
                                        {{ $c->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="og_image_id"
                                ...
                                @foreach ($images as $img)
                                    <option value="{{ $img->id }}"
                                        {{ old('og_image_id', $entry->og_image_id) == $img->id ? 'selected' : '' }}>
                                        {{ $img->title ?: $img->alt ?: 'Image #' . $img->id }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="status" required
                                ...
                                @foreach (['draft', 'private', 'published', 'archived'] as $s)
                                    <option value="{{ $s }}"
                                        {{ old('status', $entry->status) === $s ? 'selected' : '' }}>
                                        {{ ucfirst($s) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label
                                style="display:block;font-size:12px;font-weight:500;margin-bottom:6px;color:var(--cms-text-secondary);">Visibility</label>
                            <select name="visibility" required
                                style="width:100%;box-sizing:border-box;background:var(--cms-input-bg);border:1px solid var(--cms-input-border);border-radius:5px;padding:8px 12px;font-size:13px;color:var(--cms-input-text);">
                                <option value="public"
                                    {{ old('visibility', $entry->visibility) === 'public' ? 'selected' : '' }}>Public
                                </option>
                                <option value="unlisted"
                                    {{ old('visibility', $entry->visibility) === 'unlisted' ? 'selected' : '' }}>
                                    Unlisted</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="datetime-local" name="published_at"
                                value="{{ old('published_at', $entry->published_at?->format('Y-m-d\TH:i')) }}"
                                style="width:100%;box-sizing:border-box;background:var(--cms-input-bg);border:1px solid var(--cms-input-border);border-radius:5px;padding:8px 12px;font-size:13px;color:var(--cms-input-text);">
                        </div>
                        <div style="display:flex;align-items:end;">
                            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
                                <input type="checkbox" name="featured" value="1"
                                    {{ old('featured', $entry->featured) ? 'checked' : '' }}
            ...
            {{-- Broadcast section (shown when at least one platform is enabled globally) --}}
            @php
            ...
            <div style="display:flex;gap:10px;align-items:center;">
                <button type="submit"
                    ...
                    Entry</button>
                <a href="{{ route('panel.entries.index') }}"
                ...
                @if ($entry->exists)
                    <a href="{{ in_array($entry->status, ['draft', 'archived']) ? \Illuminate\Support\Facades\URL::temporarySignedRoute('site.show', now()->addHours(24), [$entry->contentType->slug, $entry->slug]) : route('site.show', [$entry->contentType->slug, $entry->slug]) }}"
                        target="_blank"
                        style="margin-left:auto;font-size:12px;color:var(--cms-accent);text-decoration:none;">Preview
                        on frontend →</a>
                @endif
            </div>
        </form>
