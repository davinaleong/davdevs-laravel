<x-cms-layout title="Settings">
    <div style="margin-bottom:24px;">
        <h1
            ...
            Settings</h1>
        <p style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--cms-text-muted);">Site-wide
            configuration, grouped by area</p>
    </div>
    ...
    @php
        $groupLabels = [
            ...
            'api' => 'Headless API',
            'ai' => 'AI Provider',
            'broadcast' => 'Broadcast (Social Media)',
        ];
        $keyLabels = [
            ...
            'ai_model' => 'Model',
            // broadcast
            'broadcast_linkedin_enabled' => 'LinkedIn — enabled',
            'broadcast_linkedin_token' => 'LinkedIn — access token',
            'broadcast_linkedin_urn' => 'LinkedIn — member URN (e.g. urn:li:person:XXXXX)',
            'broadcast_facebook_enabled' => 'Facebook — enabled',
            'broadcast_facebook_token' => 'Facebook — page access token',
            'broadcast_facebook_page_id' => 'Facebook — page ID',
            'broadcast_instagram_enabled' => 'Instagram — enabled',
            'broadcast_instagram_token' => 'Instagram — access token',
            'broadcast_instagram_user_id' => 'Instagram — business user ID',
            'broadcast_threads_enabled' => 'Threads — enabled',
            'broadcast_threads_token' => 'Threads — access token',
    ...
    <form method="POST" action="{{ route('panel.settings.update') }}">
        ...
        @foreach ($settings as $group => $items)
            <div
                style="background:var(--cms-bg-surface);border:1px solid var(--cms-border);border-radius:8px;margin-bottom:20px;overflow:hidden;">
                <div
                    style="padding:14px 20px;border-bottom:1px solid var(--cms-border);font-family:'Inter',sans-serif;font-size:13px;font-weight:600;color:var(--cms-text-primary);">
                ...
                <div style="padding:20px;display:flex;flex-direction:column;gap:16px;">
                    @foreach ($items as $setting)
                        <div>
                            <label
                                style="display:block;font-size:12px;font-weight:500;margin-bottom:6px;color:var(--cms-text-secondary);">
                                {{ $keyLabels[$setting->key] ?? $setting->key }}
                            </label>

                            @if ($setting->key === 'ai_api_key')
                                <input type="password" name="{{ $setting->key }}"
                                    placeholder="{{ $setting->value ? '••••••••••••••••  (set — leave blank to keep)' : 'sk-...' }}"
                                    autocomplete="off"
                                ...
                                <p style="font-size:11px;color:var(--cms-text-muted);margin-top:4px;">Stored encrypted.
                                    Never displayed once set — leave blank on save to keep the current key.</p>
        ...
        <div style="display:flex;gap:10px;">
            <button type="submit"
                style="background:var(--cms-btn-primary-bg);color:var(--cms-btn-primary-text);border:none;border-radius:5px;padding:10px 20px;font-family:'Inter',sans-serif;font-size:13px;font-weight:500;cursor:pointer;">Save
                Settings</button>
        </div>
    </form>
    ...
    <form method="POST" action="{{ route('panel.settings.flush-cache') }}" style="margin-top:12px;">
        @csrf
        <button type="submit"
            style="background:var(--cms-btn-secondary-bg);color:var(--cms-btn-secondary-text);border:1px solid var(--cms-btn-secondary-border);border-radius:5px;padding:7px 14px;font-family:'Inter',sans-serif;font-size:12px;cursor:pointer;">Flush
