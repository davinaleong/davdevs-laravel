<x-cms-layout title="Edit Image">
    ...
    <div style="display:flex;gap:24px;max-width:900px;">
        <div style="flex:0 0 240px;">
            <img src="{{ $image->url }}" alt="{{ $image->alt }}"
                style="width:100%;border-radius:8px;border:1px solid var(--cms-border);">
            @if ($usageEntries->count() || $usagePublications->count())
                <div style="margin-top:12px;font-size:12px;color:var(--cms-text-muted);">
                    ...
                    <ul style="margin:6px 0 0;padding-left:16px;">
                        @foreach ($usageEntries as $e)
                            <li><a href="{{ route('panel.entries.edit', $e) }}"
                                    style="color:var(--cms-accent);">{{ $e->title }}</a></li>
                        @endforeach
                        @foreach ($usagePublications as $p)
                            <li><a href="{{ route('panel.publications.edit', $p) }}"
                                    style="color:var(--cms-accent);">{{ $p->title }}</a></li>
                        @endforeach
                    </ul>
                </div>
            @else
        ...
        <form method="POST" action="{{ route('panel.images.update', $image) }}" enctype="multipart/form-data"
            style="flex:1;">
            @csrf @method('PUT')
            <div
                ...
                <div>
                    <label
                        style="display:block;font-size:12px;font-weight:500;margin-bottom:6px;color:var(--cms-text-secondary);">Replace
                        file (optional)</label>
                ...
                <div>
                    <label
                        style="display:block;font-size:12px;font-weight:500;margin-bottom:6px;color:var(--cms-text-secondary);">Alt
                        text</label>
                ...
                <div>
                    <label
                        style="display:block;font-size:12px;font-weight:500;margin-bottom:6px;color:var(--cms-text-secondary);">Caption</label>
                    <textarea name="caption" rows="2"
                        style="width:100%;box-sizing:border-box;background:var(--cms-input-bg);border:1px solid var(--cms-input-border);border-radius:5px;padding:8px 12px;font-size:13px;color:var(--cms-input-text);">{{ $image->caption }}</textarea>
                </div>
                <div>
                    <label
                        style="display:block;font-size:12px;font-weight:500;margin-bottom:6px;color:var(--cms-text-secondary);">Credit</label>
                    <input type="text" name="credit" value="{{ $image->credit }}"
                        style="width:100%;box-sizing:border-box;background:var(--cms-input-bg);border:1px solid var(--cms-input-border);border-radius:5px;padding:8px 12px;font-size:13px;color:var(--cms-input-text);">
                </div>
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
                    <input type="checkbox" name="qr_code" value="1" {{ $image->qr_code ? 'checked' : '' }}
    ...
    <form method="POST" action="{{ route('panel.images.destroy', $image) }}" style="margin-top:24px;"
        onsubmit="return confirm('Delete this image permanently?')">
        @csrf @method('DELETE')
        <button type="submit"
            style="background:var(--cms-btn-danger-bg);color:var(--cms-btn-danger-text);border:none;border-radius:5px;padding:7px 14px;font-size:12px;cursor:pointer;">Delete
            Image</button>
    </form>
</x-cms-layout>
