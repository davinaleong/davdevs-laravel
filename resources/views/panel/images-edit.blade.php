    ...
    <div style="display:flex;gap:24px;max-width:900px;">
        ...
        <form method="POST" action="{{ route('panel.images.update', $image) }}" enctype="multipart/form-data" style="flex:1;">
            ...
            <div style="display:flex;gap:10px;margin-top:16px;">
                <button type="submit" style="background:var(--cms-btn-primary-bg);color:var(--cms-btn-primary-text);border:none;border-radius:5px;padding:8px 16px;font-size:13px;font-weight:500;cursor:pointer;">Save Changes</button>
                <a href="{{ route('panel.images.index') }}" style="background:var(--cms-btn-secondary-bg);border:1px solid var(--cms-btn-secondary-border);color:var(--cms-btn-secondary-text);padding:8px 16px;border-radius:5px;font-size:13px;text-decoration:none;">Cancel</a>
            </div>
        </form>
    </div>
