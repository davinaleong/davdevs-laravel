<x-cms-layout title="{{ $layout->exists ? 'Edit Layout' : 'New Layout' }}">
    ...
    <form method="POST" action="{{ $layout->exists ? route('panel.layouts.update', $layout) : route('panel.layouts.store') }}" style="max-width:520px;">
        ...
        <div style="display:flex;gap:10px;margin-top:16px;">
            ...
            <a href="{{ route('panel.layouts.index') }}" style="background:var(--cms-btn-secondary-bg);border:1px solid var(--cms-btn-secondary-border);color:var(--cms-btn-secondary-text);padding:10px 20px;border-radius:5px;font-size:13px;text-decoration:none;">Cancel</a>
        </div>
    </form>
</x-cms-layout>
