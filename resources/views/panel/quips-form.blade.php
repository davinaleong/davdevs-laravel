<x-cms-layout title="{{ $quip->exists ? 'Edit Quip' : 'New Quip' }}">
    ...
    <form method="POST" action="{{ $quip->exists ? route('panel.quips.update', $quip) : route('panel.quips.store') }}" style="max-width:520px;" x-data="{ variant: '{{ old('variant', $quip->variant ?? 'qa') }}' }">
        ...
        <div style="display:flex;gap:10px;margin-top:16px;">
            ...
            <a href="{{ route('panel.quips.index') }}" style="background:var(--cms-btn-secondary-bg);border:1px solid var(--cms-btn-secondary-border);color:var(--cms-btn-secondary-text);padding:8px 16px;border-radius:5px;font-size:13px;text-decoration:none;">Cancel</a>
        </div>
    </form>
</x-cms-layout>
