...
<body data-cms data-theme="light" x-data="{ sidebarOpen: true, theme: localStorage.getItem('cms_theme') || 'light' }"
      x-init="$el.setAttribute('data-theme', theme); document.documentElement.setAttribute('data-theme', theme)"
      :data-theme="theme" style="background:var(--cms-bg-page);color:var(--cms-text-primary)">

    {{-- Topbar --}}
    <header style="background:var(--cms-topbar-bg);color:var(--cms-topbar-text);height:40px;display:flex;align-items:center;padding:0 16px;gap:12px;position:fixed;top:0;left:0;right:0;z-index:50;">
        <button @click="sidebarOpen = !sidebarOpen" style="background:none;border:none;cursor:pointer;color:inherit;padding:4px;">
            <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M3 12h18M3 6h18M3 18h18"/></svg>
        </button>
        <span style="font-family:'JetBrains Mono',monospace;font-size:11px;font-weight:500;letter-spacing:0.04em;">~/dav/devs cms</span>
        <span style="background:var(--cms-topbar-badge-bg);color:var(--cms-topbar-badge-text);font-family:'JetBrains Mono',monospace;font-size:9px;padding:1px 6px;border-radius:3px;font-weight:500;">admin</span>
        <div style="margin-left:auto;display:flex;align-items:center;gap:12px;">
            {{-- Theme toggle --}}
            <button @click="theme = theme === 'light' ? 'dark' : 'light'; localStorage.setItem('cms_theme', theme); $el.closest('[data-cms]').setAttribute('data-theme', theme); document.documentElement.setAttribute('data-theme', theme)"
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" style="background:none;border:none;cursor:pointer;color:inherit;font-family:'Inter',sans-serif;font-size:11px;opacity:0.7;text-decoration:underline;">logout</button>
            </form>
        </div>
    </header>
    ...
    <div style="display:flex;padding-top:40px;min-height:100vh;">
        ...
        <nav x-show="sidebarOpen" x-transition
            ...
            <div style="padding:16px 0;">
                ...
                @foreach($navGroups as $group => $items)
                    <div style="padding:8px 16px 4px;font-family:'Inter',sans-serif;font-size:10px;font-weight:600;color:var(--cms-text-muted);text-transform:uppercase;letter-spacing:0.08em;">{{ $group }}</div>
        ...
        <main :style="sidebarOpen ? 'margin-left:200px' : 'margin-left:0'" style="flex:1;padding:24px;transition:margin-left 200ms ease;">
            ...
            @endif
            @if(session('error'))
                <div style="background:var(--cms-error-bg);border-left:3px solid var(--cms-error);padding:10px 14px;border-radius:5px;margin-bottom:16px;font-size:13px;color:var(--cms-error);">
                    {{ session('error') }}
                </div>
            @endif
            {{ $slot }}
