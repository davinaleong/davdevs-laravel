...
<body x-data="{ theme: localStorage.getItem('theme') || 'dark', searchOpen: false }"
    ...
    <div id="reading-progress" style="position:fixed;top:0;left:0;height:2px;background:var(--progress-fill);width:0%;z-index:60;transition:width 100ms;"></div>

    {{-- Nav --}}
    <header style="background:var(--nav-bg);backdrop-filter:blur(8px);border-bottom:0.5px solid var(--border-default);height:48px;position:sticky;top:0;z-index:50;">
        <div style="max-width:1280px;margin:0 auto;padding:0 36px;height:100%;display:flex;align-items:center;justify-content:space-between;">
            <a href="{{ route('home') }}" style="font-family:'JetBrains Mono',monospace;font-size:13px;color:var(--text-primary);text-decoration:none;">
    ...
    <div x-show="searchOpen" x-cloak @keydown.escape.window="searchOpen = false"
         ...
         x-data="{ q: '', results: { entries: [], publications: [] } }">
        <div style="background:var(--search-bg);border-radius:10px;width:100%;max-width:560px;box-shadow:var(--shadow-search);">
            ...
            <div style="max-height:400px;overflow-y:auto;padding:8px;">
                <template x-for="item in [...results.entries, ...results.publications]">
                    <a :href="item.url" style="display:block;padding:10px 12px;text-decoration:none;border-radius:6px;">
                        <div style="font-size:13px;color:var(--text-primary);" x-text="item.title"></div>
                        <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--text-muted);" x-text="item.type"></div>
                    </a>
                </template>
            </div>
        </div>
    ...
    <footer style="background:var(--footer-bg);border-top:0.5px solid var(--footer-border);padding:24px 36px;">
        <div style="max-width:1280px;margin:0 auto;display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:16px;">
            <span style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--footer-text);">~/dav/<span style="color:var(--accent);">devs</span></span>
            <div style="display:flex;gap:14px;">
                @foreach(\App\Models\Link::where('category', 'social')->where('active', true)->orderBy('sort_order')->get() as $link)
                <a href="{{ $link->url }}" target="{{ $link->target }}" rel="{{ $link->rel }}" style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--text-faint);text-decoration:none;">{{ $link->label }}</a>
                @endforeach
            </div>
            <div style="text-align:right;">
                ...
                <span style="font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--footer-text);">
                    perf {{ $settings['lh_performance'] ?? '—' }} · a11y {{ $settings['lh_accessibility'] ?? '—' }} · seo {{ $settings['lh_seo'] ?? '—' }}
                </span>
                @endif
            </div>
        </div>
